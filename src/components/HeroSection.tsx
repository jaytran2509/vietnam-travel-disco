...
export function HeroSection({
  ...
  return (
    <div 
      className="relative h-[320px] md:h-[360px] flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/90 to-primary"
    >
      <div
      ...
      <div className="relative z-10 w-full max-w-3xl px-4 text-center space-y-6">
        <div className="space-y-3">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white drop-shadow-lg leading-[1.15] tracking-tight">
            {t.hero.title}
          </h1>
          <p className="text-base md:text-lg text-white/95 drop-shadow-md max-w-xl mx-auto font-medium leading-relaxed">
            {t.hero.subtitle}
          </p>
        </div>

        <div className="max-w-xl mx-auto space-y-3">
          <div className="relative group bg-white rounded-xl p-1 shadow-xl">
            <div className="relative">
              <MagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-foreground/60 transition-colors group-focus-within:text-primary z-10" weight="bold" />
              <Input
                placeholder={t.hero.searchPlaceholder}
                value={searchQuery}
                onChange={(e) => onSearchChange(e.target.value)}
                className="pl-11 pr-4 h-12 text-[15px] bg-white border-0 shadow-none transition-all duration-200 focus-visible:ring-0 rounded-lg font-medium"
              />
            </div>
          </div>
